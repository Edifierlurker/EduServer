 ...
 * 
 */
public class SearchGroupStudentListAction extends Business
{
    ...
    public void execute()
    {
        ...
        log.info("搜索群组成员列表列表接口接收到的参数：userindex=" + userIndex + ",groupIndex=" + groupIndex + ",keyword=" + keyword);
        ...
        try
        {
            ...
            resultJobj.put("desc", "搜索群组成员列表列表失败");

            // 所有的群组列表
            List<TGroupinfoAttendMap> list = groupFS.searchGroupUserList(Long.parseLong(groupIndex), keyword);
        ...
        catch (Exception e)
        {
            ...
            log.info("搜索群组成员列表列表接口异常", e);

        }
        finally
        {
            log.info("搜索群组成员列表列表接口响应的Json类型的结果字符串：" + resultJobj.toString());
    ...
    private boolean confirm()
    {
        ...
        try
        {
            ...
            if (Tools.isNull(groupIndex) || Tools.isNull(keyword))
            {
                return false;
            }
