 ...
 * 
 */
public class SearchGroupListAction extends Business
{
    ...
    public void execute()
    {
        ...
        log.info("获取群组列表接口接收到的参数：userindex=" + userIndex + ",pageNum=" + pageNum + ",count=" + count + ",keyword="
                + keyword + ",longitude=" + longitude + ",latitude=" + latitude);
        ...
        try
        {
            ...
            // 所有的群组列表
            TableDataInfo tabledata = groupFS.getGroupListByKeyword(keyword, longitude, latitude, pageNum, count);
    ...
    private boolean confirm()
    {
        ...
        try
        {
            ...
            if (Tools.isNull(pageNumStr) || Tools.isNull(countStr) || Tools.isNull(keyword) || Tools.isNull(longitude)
                    || Tools.isNull(latitude))
            {
                return false;
            }
